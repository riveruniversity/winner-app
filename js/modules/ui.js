...
window.UI = (function() {
  ...
  async function populateQuickSelects() {
    try {
      ...
      if (quickPrizeSelect) {
        ...
        if (window.settings.selectedPrizeId) {
  ...
  function applyVisibilitySettings() {
    ...
    if (totalEntriesCard) {
      totalEntriesCard.style.display = window.settings.hideEntryCounts ? 'none' : 'block';
    }
  }
  ...
  function updateSelectionInfo() {
    ...
    window.settings.selectedListId = quickListSelect.value;
    window.settings.selectedPrizeId = quickPrizeSelect.value;
    window.settings.winnersCount = parseInt(quickWinnersCount.value) || 1;
