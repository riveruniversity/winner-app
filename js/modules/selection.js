...
window.Selection = (function() {
  ...
  function showCountdown(winnersCount, selectedPrize, postCountdownDisplayMode = 'all-at-once') {
    ...
    const interval = setInterval(() => {
      ...
      if (count > 0) {
        countdownNumber.textContent = count;
        if (window.settings.enableSoundEffects) {
          playSound('countdown');
        }
      } else {
    ...
    try {
      ...
      const selectedEntries = await new Promise((resolve, reject) => {
        ...
        randomWorker.postMessage({
          type: 'select',
          entries: window.currentList.entries,
      ...
      const winners = selectedEntries.map((entry, index) => ({
        ...
        ...entry.data,
        displayName: Lists.formatDisplayName(entry, window.currentList.metadata.nameConfig),
        prize: selectedPrize.name,
        timestamp: Date.now(),
        originalEntry: entry,
        listId: window.currentList.listId || window.currentList.metadata.listId,
        position: index + 1,
        historyId: historyId
      }));
      ...
      const historyEntry = {
        historyId: historyId,
        listId: window.currentList.listId || window.currentList.metadata.listId,
        listName: window.currentList.metadata.name,
        prize: selectedPrize.name,
      ...
      if (window.settings.preventDuplicates) {
        window.currentList.entries = window.currentList.entries.filter(entry =>
  ...
  function playSound(type) {
    if (!window.settings.enableSoundEffects) return;
